...
export class AddSprintComponent implements OnInit, OnDestroy {
  ...
  onSubmit(): void {
    ...
    const sprintToAdd: SprintToAdd = new SprintToAdd();
    sprintToAdd.name = this.form.controls.name.value;
    sprintToAdd.taskId = +this.route.snapshot.paramMap.get('taskId');

    sprintToAdd.active = this.active;
    sprintToAdd.finished = this.finished;

    const addSprintSub = this.sprintService.addAll(
      {
        sprint: sprintToAdd,
